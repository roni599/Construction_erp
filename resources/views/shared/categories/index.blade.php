...
@section('content')
    @php
        $role = auth()->user()->role === 'admin' ? 'admin' : 'manager';
    @endphp

    <div class="flex-between" style="margin-bottom: 32px;">
        <div>
            <h2>Expense Categories</h2>
            <p style="color: var(--text-secondary);">Manage categories for project expenses</p>
        </div>
        <button class="btn btn-primary" onclick="document.getElementById('createCategoryForm').style.display='block'">
    ...
    <div id="createCategoryForm" class="glass-panel animate-fade-in" style="display: none; margin-bottom: 32px; position: relative;">
        ...
        <form method="POST" action="{{ route($role . '.categories.store') }}">
            @csrf
    ...
    <div class="glass-panel" style="margin-bottom: 24px; padding: 16px;">
        <form method="GET" action="{{ route($role . '.categories.index') }}" style="display: flex; gap: 16px; align-items: flex-end;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <a href="{{ route($role . '.categories.index') }}" class="btn btn-outline">Clear</a>
            </div>
        </form>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                ...
                                <form id="delete-cat-{{ $category->id }}" action="{{ route($role . '.categories.destroy', $category->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
    ...
    <div id="editCategoryModal" class="sidebar-overlay" style="display: none; align-items: center; justify-content: center; z-index: 2000;">
        <div class="glass-panel animate-slide-up-custom" style="width: 100%; max-width: 500px; padding: 32px; position: relative;">
    ...
    <script>
        function openEditModal(id, name, description, isActive) {
            ...
            form.action = `/${role}/categories/${id}`;
