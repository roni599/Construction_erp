...
@section('title', 'Report: Fund Transferred PM')
...
@section('content')
    ...
    <div class="flex-between no-print" style="margin-bottom: 32px;">
        <div>
            <h2>Fund Transferred to PM Report</h2>
        </div>
    </div>

    <!-- Filters -->
    <div class="glass-panel no-print" style="margin-bottom: 24px; padding: 20px;">
        <form method="GET" action="{{ route('admin.reports.fund_transferred') }}" style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr auto; gap: 16px; align-items: end;">
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <a href="{{ route('admin.reports.fund_transferred') }}" class="btn btn-outline" style="border-color: var(--danger); color: var(--danger);">Clear</a>
            </div>
            @if(count($report_data) > 0)
            <div style="display: flex; gap: 10px;">
                <a href="{{ route('admin.reports.print', array_merge(request()->query(), ['report_type' => 'fund_pm'])) }}" target="_blank" class="btn btn-outline" style="display: flex; align-items: center; gap: 8px;">
    ...
    <div class="print-only">
        <table class="report-table" style="width: 100%; border-collapse: collapse; margin-bottom: 0;">
            <tbody>
                <tr>
                    <th style="background-color: #f8f9fa; color: #000; font-size: 20px; padding: 12px; text-transform: uppercase; border: 1px solid #000; text-align: center;">Fund Transferred to PM Report</th>
                </tr>
                <tr>
    ...
    <div class="glass-panel">
        <div class="table-wrapper">
            <table class="table" id="fund-transfer-table">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Invoice No</th>
                ...
                <tbody>
                    ...
                    @forelse($report_data as $data)
                        ...
                        <tr>
                            <td>{{ $data->fund_date->format('Y-m-d') }}</td>
                            <td style="font-family: monospace;">
                                <a href="{{ route('admin.projects.funds.invoice', $data->id) }}" target="_blank" style="color: var(--accent-yellow); text-decoration: none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                                    {{ $data->invoice_no ?? 'N/A' }}
                                </a>
                            </td>
                            ...
                            <td style="text-align: right; color: var(--accent-blue); font-weight: bold;">Tk. {{ number_format($data->amount, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                ...
                <tfoot>
                    <tr style="border-top: 2px solid var(--border-color);">
                        <th colspan="5" style="text-align: right; font-size: 16px;">Total Disbursed:</th>
                        <th style="text-align: right; color: var(--accent-blue); font-size: 16px;">Tk. {{ number_format($total, 2) }}</th>
                    </tr>
                </tfoot>
                @endif
            </table>
    ...
    <script>
        function exportToExcel() {
            ...
            const projectTitle = "FUND TRANSFERRED PM REPORT";
            ...
            const table = document.getElementById("fund-transfer-table");
            ...
            // Footer
            const footerData = [['Total Disbursed:', '', '', '', '', "Tk. {{ number_format($total ?? 0, 2) }}"]];
            ...
            XLSX.writeFile(wb, "Fund_Transferred_Report.xlsx");
        }
        ...
        function exportToPDF() {
            ...
            const body = [];
            document.querySelectorAll('#fund-transfer-table tbody tr').forEach(row => {
                const rowData = [];
            ...
            const foot = [[{ content: 'Total Disbursed:', colSpan: 5, styles: { halign: 'right' } }, { content: 'Tk. {{ number_format($total ?? 0, 2) }}', styles: { halign: 'right', fontStyle: 'bold' } }]];

            doc.autoTable({
                head: [[{content:'Date'}, {content:'Invoice'}, {content:'Project'}, {content:'Manager'}, {content:'Method'}, {content:'Amount', styles:{halign:'right'}}]],
                body: body,
                foot: foot,
            ...
            doc.save("Fund_Transferred_Report.pdf");
        }
    </script>
@endsection
