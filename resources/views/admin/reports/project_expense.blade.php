...
@section('title', 'Report: Project Expense')
...
@section('content')
    ...
    <div class="flex-between no-print" style="margin-bottom: 32px;">
        <div>
            <h2>Project Expense Report</h2>
        </div>
    </div>

    <!-- Filters -->
    <div class="glass-panel no-print" style="margin-bottom: 24px; padding: 20px;">
        <form method="GET" action="{{ route('admin.reports.project_expense') }}">
            ...
            <div class="btn-group" style="flex-wrap: wrap; flex-direction: row !important; justify-content: space-between;">
                <div class="btn-group" style="flex: 1; min-width: 200px; flex-direction: row !important;">
                    ...
                    <a href="{{ route('admin.reports.project_expense') }}" class="btn btn-outline" style="flex: 1; border-color: var(--danger); color: var(--danger);">Clear</a>
                </div>

                @if(count($report_data) > 0)
                <div class="btn-group" style="flex: 1.5; min-width: 300px; justify-content: flex-end; flex-direction: row !important;">
                    <a href="{{ route('admin.reports.print', array_merge(request()->query(), ['report_type' => 'expense'])) }}" target="_blank" class="btn btn-outline" style="flex: 1; font-size: 13px; padding: 8px 12px !important;">
    ...
    <div class="print-only">
        <table class="report-table" style="width: 100%; border-collapse: collapse; margin-bottom: 0;">
            <tbody>
                <tr>
                    <th style="background-color: #f8f9fa; color: #000; font-size: 20px; padding: 12px; text-transform: uppercase; border: 1px solid #000; text-align: center;">Project Expense Report</th>
                </tr>
                <tr>
    ...
    <div class="glass-panel">
        <div class="table-wrapper">
            <table class="table" id="expense-report-table">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Invoice No</th>
                    ...
                    @forelse($report_data as $data)
                        ...
                        <tr>
                            ...
                            <td style="font-family: monospace;">
                                <a href="{{ route('shared.expenses.invoice', $data->id) }}" target="_blank" style="color: var(--accent-yellow); text-decoration: none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                                    {{ $data->invoice_no ?? 'N/A' }}
                                </a>
                            </td>
                            <td>{{ $data->project->project_name }}</td>
                            <td>
                                <span class="badge" style="background: rgba(255, 171, 0, 0.1); color: var(--accent-yellow); border: 1px solid rgba(255, 171, 0, 0.2);">
                                    {{ $data->category->name }}
                                </span>
                            </td>
                            <td>
    ...
    <script>
        function exportToExcel() {
            ...
            const projectTitle = "PROJECT EXPENSE REPORT";
            ...
            XLSX.writeFile(wb, "Project_Expenses_Report.xlsx");
        }
        ...
        function exportToPDF() {
            ...
            const body = [];
            document.querySelectorAll('#expense-report-table tbody tr').forEach(row => {
                const rowData = [];
