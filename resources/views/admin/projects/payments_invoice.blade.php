    ...
    <div class="invoice-box">
        <div class="header">
            ...
            <div class="invoice-info">
                ...
                <p>Invoice #: <strong>{{ $payment->invoice_no }}</strong></p>
        ...
        <div class="details">
            <div>
                <h3>Received From</h3>
                <p><strong>{{ $payment->project->client_name }}</strong></p>
                <p>Project: {{ $payment->project->project_name }}</p>
                @if($payment->project->client_phone) <p>Phone: {{ $payment->project->client_phone }}</p> @endif
            </div>
            <div>
                <h3>Payment Details</h3>
                <p>Method: {{ ucfirst(str_replace('_', ' ', $payment->payment_method)) }}</p>
                @if($payment->reference_no) <p>Reference: {{ $payment->reference_no }}</p> @endif
        ...
        <table class="table">
            ...
            <tbody>
                <tr>
                    <td>Project Installment / Payment for {{ $payment->project->project_name }}</td>
        ...
        <div class="total">
            Total Amount: Tk. {{ number_format($payment->amount, 2) }}
        </div>

        @if($payment->note)
            <div style="margin-top: 30px;">
                <h3 style="font-size: 14px; color: #7f8c8d; text-transform: uppercase;">Note:</h3>
                <p style="font-style: italic; color: #34495e;">{{ $payment->note }}</p>
            </div>
        @endif
