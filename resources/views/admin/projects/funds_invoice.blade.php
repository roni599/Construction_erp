...
<head>
    ...
    <title>Disbursement Invoice - {{ $fund->invoice_no }}</title>
    <style>
    ...
    <div class="invoice-box">
        <div class="header">
            ...
            <div class="invoice-info">
                <h1>DISBURSEMENT INVOICE</h1>
                <p>Reference #: <strong>{{ $fund->invoice_no }}</strong></p>
                <p>Date: {{ $fund->fund_date->format('d M, Y') }}</p>
            </div>
        </div>
        ...
        <div class="details">
            <div>
                <h3>Disbursed To</h3>
                <p><strong>{{ $fund->employee->name }}</strong></p>
                <p>Role: Project Manager</p>
                <p>Project: {{ $fund->project->project_name }}</p>
            </div>
            <div>
                <h3>Transaction Details</h3>
                <p>Method: {{ ucfirst(str_replace('_', ' ', $fund->payment_method)) }}</p>
                <p>Disbursed By: {{ $fund->givenBy->name }}</p>
            </div>
        </div>
        ...
        <table class="table">
            ...
            <tbody>
                <tr>
                    <td>Project Operational Fund for {{ $fund->project->project_name }}</td>
                    <td style="text-align: right;">Tk. {{ number_format($fund->amount, 2) }}</td>
                </tr>
            </tbody>
        </table>

        <div class="total">
            Total Disbursed: Tk. {{ number_format($fund->amount, 2) }}
        </div>

        @if($fund->note)
            <div style="margin-top: 30px;">
                <h3 style="font-size: 14px; color: #7f8c8d; text-transform: uppercase;">Note:</h3>
                <p style="font-style: italic; color: #34495e;">{{ $fund->note }}</p>
            </div>
        @endif

        <div class="footer">
            <p>This is an official disbursement invoice. Please keep for your records.</p>
            <p>&copy; {{ date('Y') }} {{ $adminInfo->business_name ?? 'Construction ERP' }}</p>
        </div>
    </div>

    <div class="btn-container">
        <button class="no-print-btn" onclick="window.print()">Print Advice</button>
    </div>
</body>
</html>
