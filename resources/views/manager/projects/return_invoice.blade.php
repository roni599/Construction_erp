...
<head>
    ...
    <title>Fund Return Receipt - {{ $return->invoice_no }}</title>
    <style>
    ...
    <div class="invoice-box">
        <div class="header">
            ...
            <div class="invoice-info">
                <h1>FUND RETURN RECEIPT</h1>
                <p>Reference #: <strong>{{ $return->invoice_no }}</strong></p>
                <p>Date: {{ $return->return_date->format('d M, Y') }}</p>
            </div>
        </div>
        ...
        <div class="details">
            <div>
                <h3>Returned By</h3>
                <p><strong>{{ $return->employee->name ?? 'N/A' }}</strong></p>
                <p>Role: Project Manager</p>
                <p>Project: {{ $return->project->project_name ?? 'N/A' }}</p>
            </div>
            <div>
                <h3>Transaction Details</h3>
                <p>Method: {{ ucfirst(str_replace('_', ' ', $return->payment_method)) }}</p>
                <p>Received By: {{ $return->receivedBy->name ?? 'System Admin' }} (Admin)</p>
            </div>
        </div>
        ...
        <table class="table">
            ...
            <tbody>
                <tr>
                    <td>Surplus Fund Return for {{ $return->project->project_name ?? 'Deleted Project' }}</td>
                    <td style="text-align: right;">Tk. {{ number_format($return->amount, 2) }}</td>
                </tr>
            </tbody>
        </table>

        <div class="total">
            Total Returned: Tk. {{ number_format($return->amount, 2) }}
        </div>

        @if($return->note)
            <div style="margin-top: 30px;">
                <h3 style="font-size: 14px; color: #7f8c8d; text-transform: uppercase;">Note:</h3>
                <p style="font-style: italic; color: #34495e;">{{ $return->note }}</p>
            </div>
        @endif

        <div class="footer">
            <p>This is an official receipt for returned funds. Please keep for your records.</p>
